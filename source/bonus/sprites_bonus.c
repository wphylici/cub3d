...
/*   Updated: 2020/11/02 04:27:55 by wphylici         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_pixel_sprite(t_cub *cub, t_sprites *tmp)
{
	...
	x = -1;
	get_color_and_interactive(cub, tmp);
	...
	* tmp->sprite_angle - tmp->sprite_h / 2;
	...
	while (++x < tmp->sprite_h && (x + tmp->sprite_x) <
	cub->width && !tmp->flag_invis)
	{
		y = -1;
		while (++y < tmp->sprite_h && (y + tmp->sprite_y) < cub->height)
		{
			...
			if (cub->i_t == 8)
				distance_shade(cub, cub->color, tmp->dist);
